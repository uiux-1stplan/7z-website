import { adminSessionCookie, issueAdminSession, noStoreHeaders, validAdminCredentials } from '../../lib/private-access.js';
...
export default async function handler(request, response) {
  ...
  if (!validText(clientId) || !validText(accessKey)) {
    ...
    return send(response, 401, { ok: false, error: 'Admin access not recognized' });
  }

  const credentialsAreValid = await validAdminCredentials(clientId, accessKey, process.env);
  const secret = process.env.PRIVATE_ACCESS_ADMIN_SESSION_SECRET;
