...
export const ADMIN_COOKIE_NAME = '__Host-7z_admin_session';
...
export const PRIVATE_RESOURCES = Object.freeze({
  ...
  'blueprint-html': Object.freeze({
    cookieName: '__Host-7z_blueprint_html_session',
    clientIdEnv: 'PRIVATE_ACCESS_BLUEPRINT_CLIENT_ID',
    accessKeyEnv: 'PRIVATE_ACCESS_BLUEPRINT_ACCESS_KEY',
    paths: Object.freeze(['/strategic-blueprint'])
  }),
  'blueprint-pdf': Object.freeze({
    cookieName: '__Host-7z_blueprint_pdf_session',
    clientIdEnv: 'PRIVATE_ACCESS_BLUEPRINT_CLIENT_ID',
    accessKeyEnv: 'PRIVATE_ACCESS_BLUEPRINT_ACCESS_KEY',
    paths: Object.freeze(['/api/private-documents/blueprint'])
  }),
  'tawjihi-quotation': Object.freeze({
    cookieName: '__Host-7z_tawjihi_quotation_session',
    clientIdEnv: 'PRIVATE_ACCESS_TAWJIHI_CLIENT_ID',
    accessKeyEnv: 'PRIVATE_ACCESS_TAWJIHI_ACCESS_KEY',
    paths: Object.freeze(['/tawjihi-english-quotation'])
  }),
  'oman-partnership': Object.freeze({
    cookieName: '__Host-7z_oman_partnership_session',
    clientIdEnv: 'PRIVATE_ACCESS_OMAN_CLIENT_ID',
    accessKeyEnv: 'PRIVATE_ACCESS_OMAN_ACCESS_KEY',
    paths: Object.freeze(['/oman-market-partnership'])
  }),

  'scmc-proposal': Object.freeze({
    cookieName: '__Host-7z_scmc_proposal_session',
    clientIdEnv: 'PRIVATE_ACCESS_SCMC_CLIENT_ID',
    accessKeyEnv: 'PRIVATE_ACCESS_SCMC_ACCESS_KEY',
    paths: Object.freeze(['/private-access/scmc-7z-proposal'])
  })
});

export const HUB_PUBLIC_SCOPES = Object.freeze(['silla', 'elcon', 'tawjihi-quotation', 'oman-partnership', 'scmc-proposal']);
export const HUB_ADMIN_SCOPES = Object.freeze(['silla', 'elcon', 'blueprint-html', 'blueprint-pdf', 'tawjihi-quotation', 'oman-partnership', 'scmc-proposal']);
export const ADMIN_ONLY_SCOPES = Object.freeze(['blueprint-html', 'blueprint-pdf']);

export function isAdminOnlyResource(scope) {
  return ADMIN_ONLY_SCOPES.includes(scope);
}

export function isHubPublicResource(scope) {
  return HUB_PUBLIC_SCOPES.includes(scope);
}

export function hubResourcePayload(scope) {
  const labels = {
    silla: 'Silla Hall',
    elcon: 'ELCON Arabia',
    'blueprint-html': 'Strategic Blueprint HTML',
    'blueprint-pdf': 'Strategic Blueprint PDF',
    'tawjihi-quotation': 'Tawjihi English',
    'oman-partnership': 'Oman Market',
    'scmc-proposal': 'Smile Care Medical Center'
  };
  return isResource(scope) ? { scope, label: labels[scope] || scope } : null;
}
...
export async function issueAdminSession(secret, now = Date.now()) {
  const key = await hmacKey(secret);
  if (!key) return null;
  const issuedAt = Math.floor(now / 1000);
  const payload = { a: 1, i: issuedAt, e: issuedAt + ADMIN_SESSION_MAX_AGE_SECONDS, j: randomId() };
  const encodedPayload = toBase64Url(encoder.encode(JSON.stringify(payload)));
  const signature = new Uint8Array(await crypto.subtle.sign('HMAC', key, encoder.encode(encodedPayload)));
  return `${encodedPayload}.${toBase64Url(signature)}`;
}
...
export async function verifyAdminSession(token, secret, now = Date.now()) {
  ...
  try {
    const payload = JSON.parse(decoder.decode(payloadBytes));
    const nowSeconds = Math.floor(now / 1000);
    return payload && payload.a === 1 && Number.isInteger(payload.i) && Number.isInteger(payload.e)
      && typeof payload.j === 'string' && /^[A-Za-z0-9_-]{24}$/.test(payload.j)
      && payload.i <= nowSeconds + 60 && payload.e > nowSeconds
      && payload.e - payload.i === ADMIN_SESSION_MAX_AGE_SECONDS;
  } catch {
    return false;
  }
}

export function adminSessionCookie(value, maxAge = ADMIN_SESSION_MAX_AGE_SECONDS) {
  const age = Number.isInteger(maxAge) && maxAge >= 0 ? maxAge : 0;
  return `${ADMIN_COOKIE_NAME}=${value}; Path=/; Max-Age=${age}; HttpOnly; Secure; SameSite=Strict`;
}
...
export async function validAdminCredentials(clientId, accessKey, environment) {
  if (typeof clientId !== 'string' || typeof accessKey !== 'string') return false;

  const credentialPairs = [
    [environment?.PRIVATE_ACCESS_ADMIN_KHALED_CLIENT_ID, environment?.PRIVATE_ACCESS_ADMIN_KHALED_ACCESS_KEY],
    [environment?.PRIVATE_ACCESS_ADMIN_HASAN_CLIENT_ID, environment?.PRIVATE_ACCESS_ADMIN_HASAN_ACCESS_KEY]
  ];

  const checks = credentialPairs.map(async ([expectedClientId, expectedAccessKey]) => {
    if (typeof expectedClientId !== 'string' || typeof expectedAccessKey !== 'string' || !expectedClientId || !expectedAccessKey) {
      return false;
    }
    const [clientMatches, keyMatches] = await Promise.all([
      constantTimeEqual(clientId, expectedClientId),
